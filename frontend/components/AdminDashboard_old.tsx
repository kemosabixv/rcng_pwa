...
export function AdminDashboard({ onClose }: AdminDashboardProps) {
  ...
  const loadMembers = async () => {
    try {
      ...
      setMembers([]); // Ensure state remains an array
    }
  };

  const loadCommittees = async () => {
    try {
      const response = await apiClient.getCommittees();
      const data = handleApiResponse(response);
      const committeesList = data || [];
      setCommittees(Array.isArray(committeesList) ? committeesList : []);
    } catch (err) {
      console.error("Failed to load committees:", err);
      setCommittees([]); // Ensure state remains an array
    }
  };
  ...
  const loadProjects = async () => {
    try {
      ...
      setProjects([]); // Ensure state remains an array
    }
  };

  const loadDues = async () => {
    try {
      const response = await apiClient.getDues();
      const data = handleApiResponse(response);
      const duesList = data || [];
      setDues(Array.isArray(duesList) ? duesList : []);
    } catch (err) {
      console.error("Failed to load dues:", err);
      setDues([]); // Ensure state remains an array
    }
  };
  ...
  const loadDocuments = async () => {
    try {
      ...
      setDocuments([]); // Ensure state remains an array
    }
  };
  ...
  const loadQuotations = async () => {
    try {
      ...
      const data = handleApiResponse(response);
      const quotationsList = data || [];
      setQuotations(Array.isArray(quotationsList) ? quotationsList : []);
    } catch (err) {
      console.error("Failed to load quotations:", err);
      setQuotations([]); // Ensure state remains an array
    }
  };
    ...
    try {
      ...
      if (data) {
        const transformedData: Analytics = {
          ...
          projects: {
            total: 0, // Default since backend doesn't have project stats yet
            active: 0,
            completed: 0,
          },
          financial: {
            totalDues: 0, // Default since backend doesn't have financial stats yet
            paidDues: 0,
            pendingDues: 0,
          },
          events: {
            total: 0, // Default since backend doesn't have event stats yet
            upcoming: 0,
          },
        };
      ...
      setAnalytics(null); // Set to null on error
    }
  };
  ...
  const handleCreateCommittee = async () => {
    try {
      setLoading(true);
      const response = await apiClient.createCommittee(newCommittee);
  ...
  const handleCreateDue = async () => {
    try {
      setLoading(true);
      const response = await apiClient.createDue(newDue);
      handleApiResponse(response);
  ...
  const handleCreateQuotation = async () => {
    try {
      setLoading(true);
      const response = await apiClient.createQuotation(newQuotation);
      handleApiResponse(response);
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <div className="mb-6">
            ...
            <div className="hidden sm:grid md:hidden grid-cols-4 gap-1 p-1">
              ...
              <TabsList className="grid grid-cols-5 gap-1 mt-1">
                ...
                <TabsTrigger value="blog" className="text-xs p-2">
                  Blog
                </TabsTrigger>
